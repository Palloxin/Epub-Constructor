...
`;
  ...
  <body>
    ...
    metadata.push(
      `<dc:title class="title">${this.epubSettings.title ?? ""}</dc:title>`
    );
    metadata.push(
    ...
    metadata.push(
      `<dc:rights class="rights">${this.epubSettings.author ?? ""}</dc:rights>`
    );
    metadata.push(
      `<dc:source class="source">${this.epubSettings.source ?? ""}</dc:source>`
    );
    ...
    for (var x of this.epubSettings.chapters) {
      ...
      if (!x.fileName.endsWith(".html")) x.fileName += ".html";
      manifest.push(
  ...
  <body>
      ...
      navMap.push(
        `<navPoint id="${
          ...
          x.title
        }</text> </navLabel> <content src="${x.fileName}" /></navPoint>`
      );
      index++;
